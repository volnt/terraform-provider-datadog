...
- **event_query** (Block List, Max: 1) A timeseries formula and functions events query. (see [below for nested schema](#nestedblock--variables--event_query))
...
- **data_source** (String) The data source for event platform-based queries. Valid values are `rum`.
